    ...
    feed.innerHTML = stories.length ? stories.map((s, i) => {
        ...
        <article class="news-article" id="${storyId}">
            <div class="article-header">
                ...
                <button class="share-btn-mini" onclick="shareStory('${s.title.replace(/'/g, "\\'")}', '${storyId}')">
            ...
            <button id="read-btn-${i}" class="budd-read-more" onclick="handleAction(${i}, '${Array.isArray(s.category) ? s.category[0] : s.category}')">READ STORY</button>
        </article><hr style="margin:25px 0; border:0; border-top:1px solid #eee;">`;
    }).join('') : "<p style='text-align:center;'>Feed ready.</p>";
}
...
function saveDraft() {
    const d = { title: document.getElementById('post-title').value, category: [], date: new Date().toISOString() };
    if(!newsData.drafts) newsData.drafts = []; newsData.drafts.unshift(d); saveToLocal(); showTab('drafts-tab');
}

function renderDraftsList() { const list = document.getElementById('drafts-list'); list.innerHTML = (newsData.drafts && newsData.drafts.length) ? newsData.drafts.map((d, i) => `<div class="list-item"><span>${d.title || "No Title"}</span><button onclick="deleteDraft(${i})" class="btn-del">DEL</button></div>`).join('') : "<p>Empty.</p>"; }
function deleteDraft(i) { newsData.drafts.splice(i, 1); saveToLocal(); renderDraftsList(); }
function renderManageList() { document.getElementById('manage-list').innerHTML = newsData.all.length ? newsData.all.map((s, i) => `<div class="list-item"><span>${s.title.slice(0,35)}...</span><button onclick="deletePost(${i})" class="btn-del">DEL</button></div>`).join('') : "<p>Empty.</p>"; }
...
function exportData() { const b = new Blob([JSON.stringify({ news: newsData })], { type: "application/json" }); const l = document.createElement('a'); l.href = URL.createObjectURL(b); l.download = `BUDD-HUB-Backup.json`; l.click(); }
